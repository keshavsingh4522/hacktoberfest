...
ll construct(ll ss, ll se, ll si)
{
    ...
    ll mid = (ss + se) / 2;
    tre[si] = construct(ss, mid, 2 * si + 1) ^ construct(mid + 1, se, 2 * si + 2);
    return tre[si];
}
...
ll query(ll qs, ll qe, ll ss, ll se, ll si)
{
    ...
    ll mid = (ss + se) / 2;
    return query(qs, min(qe, mid), ss, mid, 2 * si + 1) ^ query(max(mid + 1, qs), qe, mid + 1, se, 2 * si + 2);
}
