...
void solve()
{
    ...
    for (int i = 0; i < n; i++)
    {
        int l, r;
        cin >> l >> r;
        ar[i].first.first = l, ar[i].first.second = r;
        ar[i].second = i;
    }
    int ans[n + 1] = {0};
    sort(all(ar));
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        {
            last_room++;
            pq.push({-ar[i].first.second, last_room});
            ans[ar[i].second] = last_room;
        }
        else
        {
            auto x = pq.top();
            if (-x.first < ar[i].first.first)
            {
                pq.pop();
                pq.push({-ar[i].first.second, x.second});
                ans[ar[i].second] = x.second;
            }
            else
            {
                last_room++;
