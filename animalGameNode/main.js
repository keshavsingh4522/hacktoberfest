...
function appreciation() {
  let words = [
    "Yay i got it!",
    "Yeah, I am still the better one",
    "We are living in the matrix",
    "Yes Yes!! The future is machines",
  ];
...
function doRound(node) {
  if (node.question) {
    ...
    if (answer.toLowerCase() == "yes") {
      ...
      let newQuestion = readlineSync.question(
        "What is the distinguishing question between " +
          node.animalName +
          " and " +
          newAnimal +
          "?\n"
      ...
      node.no = {};
      node.yes = {};
      let newAnimalTof = readlineSync.question(
        "For " + newAnimal + " is the answer yes?\n"
      );
